...
def parse_sped_ecd(conteudo_bytes, log, progress_bar, status_text):
        ...
        try:
            if registro == "0000":
                ...
                if dc_raw not in ("D", "C"):
                    log.append(f"Aviso linha {num_linha}: I250 dc='{dc_raw}' inválido — ignorado.")
                    continue
                partida = {
    ...
    # DEBUG primeiros 10
    log.append("─" * 60)
    log.append("DEBUG — primeiros 10 lançamentos:")
...
def agrupar_partidas_por_conta(partidas):
    """Agrupa por (conta, dc) somando valores — mantém D e C separados."""
    ag = {}
    for p in partidas:
        chave = (p["conta"], p["dc"])
        if chave not in ag:
...
# ==============================
# MONTAGEM DO 6100 — V3.0
# ==============================
# Layout REAL do Domínio confirmado pelo usuário:
#
# |6000|TIPO||||
# |6100|DATA|COD_DEB||VALOR||HISTORICO|||||||   ← linha de débito
# |6100|DATA||COD_CRED|VALOR||HISTORICO|||||||  ← linha de crédito
#
# Regras:
# • Cada partida (débito ou crédito) gera UM registro 6100 separado
# • Linha de débito : COD_DEB preenchido, COD_CRED vazio
# • Linha de crédito: COD_DEB vazio,      COD_CRED preenchido
# • Todos os 6100 do mesmo lançamento compartilham data e histórico
# • 12 campos internos = 13 pipes totais (confirmado pelo layout)
# • NÃO existe registro 6110 neste layout

def _fmt6100_debito(data, conta, valor, hist):
    """Linha 6100 de DÉBITO: conta no campo 3, campo 4 vazio."""
    h = (hist or "").replace("|", " ").strip()
    return f"|6100|{data}|{conta}||{valor}||{h}|||||||"


def _fmt6100_credito(data, conta, valor, hist):
    """Linha 6100 de CRÉDITO: campo 3 vazio, conta no campo 4."""
    h = (hist or "").replace("|", " ").strip()
    return f"|6100|{data}||{conta}|{valor}||{h}|||||||"
...
def gerar_linhas_lancamento(lanc):
    """
    Gera as linhas 6000 + N×6100 para um lançamento.

    V3.0 — Arquitetura real do Domínio:
    ─────────────────────────────────────
    • 1 registro 6000 com o tipo (X/D/C/V)
    • 1 registro 6100 por PARTIDA (débito ou crédito)
    • Débitos: |6100|DATA|CONTA||VALOR||HIST|||||||
    • Créditos: |6100|DATA||CONTA|VALOR||HIST|||||||
    • NÃO gera 6110
    """
...
# ==============================
# GERADOR PRINCIPAL — V3.0
# ==============================
def gerar_dominio(ecd, log, progress_bar, status_text):
    """
    V3.0 — Gera o layout real do Domínio:
    • 1 linha 0000 com CNPJ
    • Para cada lançamento: 1×6000 + N×6100 (uma por partida)
    • Sem 6110
    """
    linhas     = []
...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        st.markdown("""### 📋 Estrutura gerada (V3.0)

            **Uma linha 6100 por partida.**
Sem registro 6110.""")
        st.markdown("---")
        st.markdown("""### 🔀 Tipos X/D/C/V
| Tipo | Regra |
|------|-------|
| **X** | 1 débito × 1 crédito → 2 linhas 6100 |
| **D** | 1 débito × N créditos → N+1 linhas 6100 |
| **C** | N débitos × 1 crédito → N+1 linhas 6100 |
| **V** | N débitos × N créditos → N+N linhas 6100 |""")
        st.markdown("---")
        st.markdown("""### ✅ Versões
**V3.0** — Arquitetura real confirmada
- 1 linha 6100 por partida (débito ou crédito)
- Sem 6110
- Débito: campo 3 preenchido, campo 4 vazio
- Crédito: campo 3 vazio, campo 4 preenchido

**V2.1** — Fix parse I200/I250

**V2.0** — Fix regra DEB/CRED
...
**V1.x** — Versões anteriores""")
    ...
    with st.expander("📖 Instruções de Uso", expanded=False):
        ...
        <h4>🔹 Como funciona (V3.0)</h4>
        <p>O layout real do Domínio usa <b>uma linha 6100 por partida</b>.
        Cada débito e cada crédito do lançamento gera um 6100 separado,
        com o campo de débito ou crédito preenchido (nunca os dois ao mesmo tempo,
        exceto tipo X que mesmo assim usa linhas separadas).</p>

        <h4>🔹 Exemplo — lançamento tipo D (1 débito × 2 créditos)</h4>
        <pre>
|6000|D||||
|6100|07/01/2022|686||4287,68||Histórico|||||||
|6100|07/01/2022||10001|4223,36||Histórico|||||||
|6100|07/01/2022||178|64,32||Histórico|||||||
        </pre>
    ...
    if converter and arquivo:
        st.session_state.log        = [f"Iniciando V3.0..."]
        st.session_state.txt_gerado = None
        st.session_state.metricas   = {}
        ...
        if linhas and ecd:
            ...
            st.session_state.nome_arquivo = f"ECD_{cnpj}_dominio_V3.txt"
            st.session_state.metricas = {
                "CNPJ"        : ecd.cnpj,
